...
Registra que el archivo ya fue subido y devuelve la URL de descarga (prefirmada).
...
El `key` debe ser el mismo devuelto por `upload-url` y debe corresponder a la organización del usuario.
...
`downloadUrl` es una URL prefirmada de lectura; caduca según `STORAGE_SIGNED_URL_EXPIRY` (ej. 3600 s).
...
- Lógica de negocio: `docs/business-logic.md`
