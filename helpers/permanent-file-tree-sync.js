/**
 * Keeps audit_tree_nodes in sync with permanent_file_sections / checklist_items
 * so tree/full is the single hierarchy under the permanent_file root.
 */
...
/**
 * Root node type seeded by tree-seed (Archivo Permanente).
 */
export async function findPermanentFileRoot(auditProjectId, transaction) {
  const { AuditTreeNode } = modelsInstance.models;
  return AuditTreeNode.findOne({
...
/**
 * Destroy subtree like tree/delete: null documents, destroy nodes. Does not touch PermanentFileSection/ChecklistItem rows.
 */
...
/**
 * Reparent a node under newParent (same project). newParent must be under permanent_file. Simplified from tree/move.
 */
...
export { TYPE_SECTION_NODE, TYPE_CHECKLIST_ITEM_NODE };
